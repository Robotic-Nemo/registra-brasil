...
export async function getPoliticianBySlug(
  ...
  const { data, error } = await supabase
    .from('politicians')
    .select('id, slug, full_name, common_name, party, party_history, state, role, role_history, photo_url, tse_id, camara_id, senado_id, wikipedia_url, is_active, created_at, updated_at')
    .eq('slug', slug)
    .single()
