...
import type { MonthlyRecap } from '@/lib/monthly-recap/compute'
import { monthLabel } from '@/lib/monthly-recap/compute'
  ...
  return (
    <article>
      <header className="mb-6">
        ...
        <h2 className="mt-1 text-2xl md:text-3xl font-bold text-gray-900 dark:text-gray-100">
          {monthLabel(recap.month)}
        </h2>
        <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">
          {recap.totalStatements.toLocaleString('pt-BR')} declarações registradas.
        </p>
      </header>
            ...
            {recap.topPoliticians.map((p) => (
              <li key={p.slug}>
                <Link href={`/politico/${p.slug}`} className="flex items-center gap-2 rounded px-1 py-0.5 hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  <span className="text-sm font-semibold tabular-nums text-indigo-700 dark:text-indigo-300">{p.count}</span>
                </Link>
              </li>
            ))}
          </ul>
            ...
            {recap.topCategories.map((c) => (
              <li key={c.slug}>
                <Link href={`/categorias/${c.slug}`} className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold tabular-nums text-indigo-700 dark:text-indigo-300">{c.count}</span>
                </Link>
              </li>
            ))}
          </ul>
