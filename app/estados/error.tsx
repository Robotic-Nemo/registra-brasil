...
export default function EstadosError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[Estados Error]', error)
  }, [error])

  return (
    <main className="max-w-lg mx-auto px-4 py-24 text-center">
      <AlertTriangle className="w-10 h-10 text-amber-400 mx-auto mb-4" />
      <h1 className="text-lg font-bold text-gray-900 mb-2">Erro ao carregar estados</h1>
      ...
      <div className="mt-8">
        <a
          href={`mailto:contato@registrabrasil.com.br?subject=${encodeURIComponent('Erro nos estados')}&body=${encodeURIComponent(`Erro: ${error.message}\nCódigo: ${error.digest ?? 'N/A'}\nData: ${new Date().toISOString()}`)}`}
