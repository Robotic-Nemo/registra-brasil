...
export async function GET(
  ...
  const rl = checkRateLimit(getRateLimitKey(request, 'estado-export'), {
  ...
  const filename = `uf-${uf.toLowerCase()}`

  if (format === 'json') {
    return NextResponse.json({
      uf, count: rows.length,
  ...
  if (format === 'csv') {
    const header = [
      'date', 'severity', 'verification', 'politician', 'party',
    ...
    const body = rows.map((r) => [
      ...
      r.politician_name, r.politician_party ?? '',
  ...
  note = {Registra Brasil; UF: ${uf}; party: ${bibtexEscape(r.politician_party ?? '')}; accessed ${new Date().toISOString().slice(0, 10)}; primary source: \\url{${r.primary_source_url}}}
}`).join('\n\n')

  return new NextResponse(
    `% Exported from Registra Brasil — estado "${uf}"\n% ${rows.length} entries\n\n${entries}\n`,
    {
      headers: {
