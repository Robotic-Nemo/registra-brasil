...
/**
 * GET /api/categorias/:slug/por-dia.json — daily (UTC) bucket counts
 * for one category (primary tag only). Status-split, gap-filled.
...
export async function GET(
  ...
  const rl = checkRateLimit(getRateLimitKey(request, 'cat-por-dia'), {
  ...
  const { series, total } = buildDailyStatusSeries(dias, rows)

  const etag = `W/"cat-por-dia-${cat.id}-${dias}-${total}"`
