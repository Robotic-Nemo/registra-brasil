...
/**
 * GET /api/estatisticas/severidade-por-dia.json — day × severity
 * level (1..5) matrix for the last N days (UTC). Gap-filled. Paired
 * with the monthly and (new) weekly variants to give dashboards full
 * temporal resolution.
 *   ?dias=7..180 (default 60)
 */
export async function GET(request: NextRequest) {
  const rl = checkRateLimit(getRateLimitKey(request, 'estat-sev-por-dia'), {
  ...
  const etag = `W/"estat-sev-dia-${dias}-${total}"`
