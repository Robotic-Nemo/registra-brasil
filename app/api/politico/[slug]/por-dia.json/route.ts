...
/**
 * GET /api/politico/:slug/por-dia.json — daily (UTC) bucket counts
 * for one politician. Status-split, gap-filled.
...
export async function GET(
  ...
  const rl = checkRateLimit(getRateLimitKey(request, 'pol-por-dia'), {
  ...
  const etag = `W/"pol-por-dia-${p.id.slice(0, 8)}-${dias}-${total}"`
