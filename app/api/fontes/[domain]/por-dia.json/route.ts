...
/**
 * GET /api/fontes/:domain/por-dia.json — daily (UTC) bucket counts
 * for statements citing this source domain. Status-split, gap-filled.
...
export async function GET(
  ...
  const rl = checkRateLimit(getRateLimitKey(request, 'fontes-por-dia'), {
  ...
  const startStr = new Date(todayUtc - (dias - 1) * 86400_000).toISOString().slice(0, 10)
  ...
  const etag = `W/"fontes-por-dia-${domain}-${dias}-${total}"`
