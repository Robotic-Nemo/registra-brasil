...
/**
 * GET /api/partidos/:slug/declaracoes.json — cursor-paginated list
 * of statements by members of a party.
 *   ?limit ?cursor ?status ?order (same shape as politico variant)
 */
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ slug: string }> },
) {
  const rl = checkRateLimit(getRateLimitKey(request, 'party-decl'), {
