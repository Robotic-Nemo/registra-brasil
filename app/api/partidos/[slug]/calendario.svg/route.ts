...
export async function GET(
  ...
  const rl = checkRateLimit(getRateLimitKey(request, 'partido-cal-svg'), {
  ...
  const corRaw = request.nextUrl.searchParams.get('cor') ?? '#7c3aed'
  const color = /^#[0-9a-fA-F]{6}$/.test(corRaw) ? corRaw : '#7c3aed'
  ...
  const etag = `W/"partido-cal-${party}-${titleYear}-${daysWithData}-${maxCount}"`
