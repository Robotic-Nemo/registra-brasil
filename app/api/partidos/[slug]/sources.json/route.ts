...
/**
 * GET /api/partidos/:slug/sources.json — top source domains + type
 * breakdown for statements by members of a party.
...
export async function GET(
  ...
  const rl = checkRateLimit(getRateLimitKey(request, 'party-sources'), {
  ...
  const { data: rows } = await (supabase.from('statements') as any)
    .select('primary_source_url, primary_source_type, politicians!inner(party)')
  ...
  const etag = `W/"ptsr-${party}-${all.length}-${domainCounts.size}"`
