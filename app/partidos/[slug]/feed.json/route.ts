...
/**
 * GET /partidos/[slug]/feed.json — per-party JSON Feed 1.1.
 */
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  if (!party || party.length > 30) {
    return NextResponse.json({ error: 'bad_party' }, { status: 400 })
  }
  ...
  const feed = buildJsonFeed({
    title: `Registra Brasil — ${party}`,
    description: `Declarações de políticos do ${party}.`,
    home_page_url: `${SITE_URL}/partidos/${party}`,
    feed_url: `${SITE_URL}/partidos/${party}/feed.json`,
    items,
  })
