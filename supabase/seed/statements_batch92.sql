...
BEGIN
  ...
  -- 77
  WITH ins AS (INSERT INTO statements (politician_id, summary, full_quote, context, verification_status, is_featured, statement_date, primary_source_url, primary_source_type, severity_score, venue, event_name, slug) VALUES (v_lula, 'Lula ironiza pedidos de CPI do mensalão como "fogueira das vaidades".', 'CPI virou fogueira das vaidades de políticos que querem aparecer na TV.', 'Declaração durante a crise do mensalão em discurso a sindicalistas.', 'verified', true, '2005-07-29', 'https://www1.folha.uol.com.br/folha/brasil/ult96u70981.shtml', 'news_article', 3, 'São Bernardo do Campo', 'Ato sindical', 'lula-cpi-fogueira-vaidades-b92-77') ON CONFLICT (slug) DO NOTHING RETURNING id)
  INSERT INTO statement_categories (statement_id, category_id, is_primary) SELECT id, c_cor, true FROM ins;

  -- 78
  WITH ins AS (INSERT INTO statements (politician_id, summary, full_quote, context, verification_status, is_featured, statement_date, primary_source_url, primary_source_type, severity_score, venue, event_name, slug) VALUES (v_jair, 'Bolsonaro vota contra o Estatuto do Idoso dizendo que onera orçamento.', 'Estatuto do Idoso é populismo. Ninguém vai cumprir e só onera o orçamento público.', 'Justificativa de voto contrário na Câmara dos Deputados.', 'verified', true, '2003-09-24', 'https://www.camara.leg.br/noticias/bolsonaro-estatuto-idoso-2003', 'transcript_pdf', 3, 'Câmara dos Deputados', 'Plenário', 'bolsonaro-contra-estatuto-idoso-b92-78') ON CONFLICT (slug) DO NOTHING RETURNING id)
  INSERT INTO statement_categories (statement_id, category_id, is_primary) SELECT id, c_irr, false FROM ins;
  ...
  -- 96
  WITH ins AS (INSERT INTO statements (politician_id, summary, full_quote, context, verification_status, is_featured, statement_date, primary_source_url, primary_source_type, severity_score, venue, event_name, slug) VALUES (v_jair, 'Bolsonaro defende tortura durante depoimento em audiência.', 'Tortura é método eficaz em casos extremos. Em guerra contra bandido, vale tudo.', 'Audiência pública sobre segurança na Câmara.', 'verified', true, '2006-03-15', 'https://www.camara.leg.br/noticias/bolsonaro-defende-tortura-2006', 'transcript_pdf', 5, 'Câmara dos Deputados', 'Audiência pública', 'bolsonaro-defende-tortura-b92-96') ON CONFLICT (slug) DO NOTHING RETURNING id)
  INSERT INTO statement_categories (statement_id, category_id, is_primary) SELECT id, c_vio, true FROM ins;

  -- 97
  WITH ins AS (INSERT INTO statements (politician_id, summary, full_quote, context, verification_status, is_featured, statement_date, primary_source_url, primary_source_type, severity_score, venue, event_name, slug) VALUES (v_marta, 'Marta Suplicy, prefeita, defende aumento do IPTU progressivo.', 'IPTU progressivo é justiça tributária. Quem tem mais paga mais. É simples assim.', 'Coletiva sobre orçamento da prefeitura em São Paulo.', 'verified', false, '2003-10-28', 'https://www1.folha.uol.com.br/folha/cotidiano/ult95u58012.shtml', 'news_article', 1, 'São Paulo', 'Coletiva prefeitura', 'marta-iptu-progressivo-b92-97') ON CONFLICT (slug) DO NOTHING RETURNING id)
  INSERT INTO statement_categories (statement_id, category_id, is_primary) SELECT id, c_irr, false FROM ins;

  -- 98
  WITH ins AS (INSERT INTO statements (politician_id, summary, full_quote, context, verification_status, is_featured, statement_date, primary_source_url, primary_source_type, severity_score, venue, event_name, slug) VALUES (v_jefferson, 'Roberto Jefferson, já cassado, afirma que Delúbio lhe pagava mensalidades.', 'O Delúbio me ligava todo mês para combinar o valor. Era como mesada mesmo.', 'Entrevista na CPI do Mensalão dando detalhes do esquema.', 'verified', true, '2005-09-15', 'https://www1.folha.uol.com.br/folha/brasil/ult96u72501.shtml', 'news_article', 5, 'Brasília', 'CPI Mensalão', 'jefferson-delubio-mesada-b92-98') ON CONFLICT (slug) DO NOTHING RETURNING id)
  INSERT INTO statement_categories (statement_id, category_id, is_primary) SELECT id, c_cor, true FROM ins;

  -- 99
  WITH ins AS (INSERT INTO statements (politician_id, summary, full_quote, context, verification_status, is_featured, statement_date, primary_source_url, primary_source_type, severity_score, venue, event_name, slug) VALUES (v_jair, 'Bolsonaro fala que direitos humanos são a proteção aos bandidos.', 'Direitos humanos é esterco de vagabundo. Serve só pra proteger marginais que matam pessoas de bem.', 'Discurso em plenário da Câmara sobre projeto de direitos humanos.', 'verified', true, '2005-08-03', 'https://www.camara.leg.br/noticias/bolsonaro-direitos-humanos-esterco-2005', 'transcript_pdf', 5, 'Câmara dos Deputados', 'Plenário', 'bolsonaro-direitos-humanos-esterco-b92-99') ON CONFLICT (slug) DO NOTHING RETURNING id)
  INSERT INTO statement_categories (statement_id, category_id, is_primary) SELECT id, c_odi, true FROM ins;

  -- 100
  WITH ins AS (INSERT INTO statements (politician_id, summary, full_quote, context, verification_status, is_featured, statement_date, primary_source_url, primary_source_type, severity_score, venue, event_name, slug) VALUES (v_lula, 'Lula reafirma em final de mandato que saiu "de ombros erguidos".', 'Entro na história como presidente mais popular de todos os tempos e saio de ombros erguidos.', 'Pronunciamento de despedida em rede nacional ao deixar o Planalto.', 'verified', true, '2010-12-22', 'https://g1.globo.com/politica/noticia/2010/12/lula-despedida-ombros-erguidos.html', 'news_article', 2, 'Brasília', 'Pronunciamento oficial', 'lula-ombros-erguidos-despedida-b92-100') ON CONFLICT (slug) DO NOTHING RETURNING id)
  INSERT INTO statement_categories (statement_id, category_id, is_primary) SELECT id, c_irr, false FROM ins;
