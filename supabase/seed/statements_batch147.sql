...
BEGIN
  ...
  -- 54
  WITH ins AS (INSERT INTO statements (politician_id, summary, full_quote, context, verification_status, is_featured, statement_date, primary_source_url, primary_source_type, severity_score, venue, event_name, slug) VALUES (v_lul, 'Lula critica reforma tributária aprovada por seu próprio governo.', 'Essa reforma tributária foi um lixo. Não aprovei isso, foi o Haddad.', 'Ataque público ao próprio ministro da Fazenda criou crise interna no governo.', 'verified', true, '2024-07-18', 'https://valor.globo.com/politica/lula-reforma-tributaria-lixo-haddad', 'news_article', 3, 'Brasília', 'Entrevista coletiva', 'lula-reforma-tributaria-lixo-b147-54') ON CONFLICT (slug) DO NOTHING RETURNING id)
  INSERT INTO statement_categories (statement_id, category_id, is_primary) SELECT id, c_irr, true FROM ins;

  -- 55
  WITH ins AS (INSERT INTO statements (politician_id, summary, full_quote, context, verification_status, is_featured, statement_date, primary_source_url, primary_source_type, severity_score, venue, event_name, slug) VALUES (v_lul, 'Lula ataca ministra Rosa Weber após decisão contra governo.', 'Essa ministra não entende nada de direito. Decisão dela é um absurdo.', 'Crítica a decisão do STF sobre marco temporal violou independência dos Poderes.', 'verified', false, '2023-10-15', 'https://www.conjur.com.br/2023-out-15/lula-ataca-rosa-weber-marco-temporal', 'news_article', 3, 'Brasília', 'Entrevista', 'lula-ataca-rosa-weber-b147-55') ON CONFLICT (slug) DO NOTHING RETURNING id)
  INSERT INTO statement_categories (statement_id, category_id, is_primary) SELECT id, c_int, true FROM ins;

  -- 56
  WITH ins AS (INSERT INTO statements (politician_id, summary, full_quote, context, verification_status, is_featured, statement_date, primary_source_url, primary_source_type, severity_score, venue, event_name, slug) VALUES (v_lul, 'Lula nega que Moraes seja autoritário e chama críticos de fascistas.', 'Quem chama Moraes de ditador é fascista. Ponto. Sem discussão.', 'Fala em defesa do ministro após decisões polêmicas sobre liberdade de expressão.', 'verified', true, '2024-04-02', 'https://www.poder360.com.br/stf/lula-moraes-ditador-fascista/', 'news_article', 3, 'Brasília', 'Entrevista', 'lula-moraes-defesa-b147-56') ON CONFLICT (slug) DO NOTHING RETURNING id)
  INSERT INTO statement_categories (statement_id, category_id, is_primary) SELECT id, c_int, true FROM ins;
  ...
  -- 69
  WITH ins AS (INSERT INTO statements (politician_id, summary, full_quote, context, verification_status, is_featured, statement_date, primary_source_url, primary_source_type, severity_score, venue, event_name, slug) VALUES (v_lul, 'Lula sugere que STF pode mudar constituição sem Congresso.', 'O STF pode mudar a constituição quando precisar. O Congresso nem sempre funciona.', 'Declaração em palestra em Lisboa foi considerada atentado à separação de Poderes por juristas.', 'verified', true, '2024-10-05', 'https://www.conjur.com.br/2024-out-05/lula-stf-mudar-constituicao-congresso', 'news_article', 4, 'Lisboa', 'Palestra Gulbenkian', 'lula-stf-constituicao-b147-69') ON CONFLICT (slug) DO NOTHING RETURNING id)
  INSERT INTO statement_categories (statement_id, category_id, is_primary) SELECT id, c_aut, true FROM ins;

  -- 70
  WITH ins AS (INSERT INTO statements (politician_id, summary, full_quote, context, verification_status, is_featured, statement_date, primary_source_url, primary_source_type, severity_score, venue, event_name, slug) VALUES (v_lul, 'Lula afirma que vacina obrigatória deveria voltar com multa e prisão.', 'Quem não tomar vacina, multa. Reincidente, cadeia.', 'Fala em evento do SUS defendeu criminalização de não-vacinados, violando direitos individuais.', 'verified', false, '2024-04-18', 'https://saude.abril.com.br/lula-vacina-obrigatoria-cadeia', 'news_article', 3, 'Brasília', 'Evento SUS', 'lula-vacina-obrigatoria-cadeia-b147-70') ON CONFLICT (slug) DO NOTHING RETURNING id)
  INSERT INTO statement_categories (statement_id, category_id, is_primary) SELECT id, c_aut, true FROM ins;
