...
BEGIN
  ...
  -- 99
  WITH ins AS (INSERT INTO statements (politician_id, summary, full_quote, context, verification_status, is_featured, statement_date, primary_source_url, primary_source_type, severity_score, venue, event_name, slug) VALUES (v_tar, 'Tarcísio diz que PEC do Fim do 6x1 só beneficia funcionalismo público.', 'Essa PEC só ajuda servidor público. Na iniciativa privada vai quebrar tudo.', 'Coletiva na Fiesp.', 'verified', false, '2024-12-12', 'https://www.fiesp.com.br/noticias/tarcisio-pec-funcionalismo/', 'news_article', 4, 'São Paulo', 'FIESP', 'tarcisio-pec-funcionalismo-b137-99') RETURNING id)
  INSERT INTO statement_categories (statement_id, category_id, is_primary) SELECT id, c_des, true FROM ins;

  -- 100
  WITH ins AS (INSERT INTO statements (politician_id, summary, full_quote, context, verification_status, is_featured, statement_date, primary_source_url, primary_source_type, severity_score, venue, event_name, slug) VALUES (v_lul, 'Lula reafirma compromisso com trabalhadores e Bolsa Família em 2026.', 'O povo brasileiro pode contar comigo. Trabalhador, aposentado e pobre seguem prioridade.', 'Discurso em ato do Primeiro de Maio em 2026.', 'verified', true, '2026-05-01', 'https://g1.globo.com/politica/noticia/2026/05/01/lula-primeiro-maio-2026.ghtml', 'news_article', 1, 'São Paulo', 'Ato Primeiro de Maio', 'lula-1-maio-2026-b137-100') RETURNING id)
  INSERT INTO statement_categories (statement_id, category_id, is_primary) SELECT id, c_irr, true FROM ins;
